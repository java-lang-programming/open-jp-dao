...
RSpec.describe Files::UfjImportCsvRow do
  ...
  let(:row_4) {
    [ '2024/12/25', '振込１', 'カ）アイ−ガイシャ', nil, "120,000", "1,000,000" ]
  }
  ...
  describe '#calculate_face_value' do
    ...
    context 'when 預かり金額' do
      let(:target_row) { row_4 }
      it 'should get face_value integer.' do
        face_value = instance.calculate_face_value
        expect(face_value).to eq(120000)
      end
    end
  end
  #
  ...
  describe '#to_ledger' do
    ...
    context 'when 預かり金額' do
      ...
      it 'should get ledger.' do
        ledger = instance.to_ledger
        expect(ledger.date.to_date).to eq(Date.new(2024, 12, 25))
        expect(ledger.name).to eq('カ）アイ−ガイシャ')
        expect(ledger.ledger_item).to eq(ledger_item_13)
        expect(ledger.face_value).to eq(120000)
        expect(ledger.recorded_amount).to eq(120000)
      end
    end
  end
  ...
  describe 'to_upsert_all_ledger' do
    context 'upsert_allを実行するためのhashを取得する' do
      it 'should get hashed data.' do
        ...
        expect(hash[:date]).to eq(Date.new(2024, 12, 2))
        expect(hash[:name]).to eq('コクミンネンキン')
        expect(hash[:ledger_item_id]).to eq(ledger_item_5.id)
        expect(hash[:face_value]).to eq(17320)
        expect(hash[:proportion_rate]).to be nil
        expect(hash[:proportion_amount]).to be nil
        expect(hash[:recorded_amount]).to eq(17320)
